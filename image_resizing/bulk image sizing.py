...
def resize_image(in_dir,infile,out_dir="resized",size=default_size):
    out_file = os.path.splitext(infile)[0]+"_resized"
    extension = os.path.splitext(infile)[1]

    try:
        img = Image.open(in_dir+"/"+infile)
        img1 = img.resize((size[0],size[1]),Image.LANCZOS)
        new_path = out_dir+"/"+out_file+extension
        img1.save(new_path)
